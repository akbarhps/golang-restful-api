...
func TestUserServiceImpl_Register(t *testing.T) {
	t.Run("register using valid input should not get panic", func(t *testing.T) {
		t.Skip("can't test this case because new user id and encrypted password generated by service and we can't compare it to the mock in repository")
		userService, userRepository := setup()

		userRepository.Mock.On("Find", context.Background(), &rawDomain).Return([]domain.User{responseDomain})
		userRepository.Mock.On("Create", context.Background(), &rawDomain).Return(nil)
...
func TestUserServiceImpl_UpdatePassword(t *testing.T) {
	t.Run("update password with valid input should not get panic", func(t *testing.T) {
		t.Skip("cant test this case because password encrypted in service and we don't know what it is")
	})
